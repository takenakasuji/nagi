...
Test-first. All 164 tests run in under four seconds and the pure layers are covered thoroughly; the `RealAppKitIntegrationTests` suite exercises the actual `NSPanel`, real `RegisterEventHotKey`, synthesised `NSEvent`s through `performKeyEquivalent`, and real `setMarkedText` conversions, so it needs a window server (30 of the 164 — the other 134 run headless in about 0.03s).
