...
**Escape is taken by the SwiftUI `.cancelAction` button in `CaptureView`, wherever focus is — including the body.** A bare Escape *is* a key equivalent: AppKit runs the key-equivalent stage before the responder chain and accepts an unmodified Escape exactly as it accepts ⌘Return (`素の Escape も key equivalent の段に流れる` measures this through a main-menu item, which needs no key window). `CapturePanel.performKeyEquivalent` passes it to `super`, the walk reaches the hidden button, it returns true, and the event stops there. `NagiTextView.cancelOperation` is therefore **not** on the live path — it only runs if something delivers Escape straight to the responder chain. **Known consequence, unfixed:** its `hasMarkedText()` guard is what was supposed to keep Escape available to the input method for cancelling a Japanese conversion, and the button does not honour it, so Escape mid-conversion closes the window. Both tests are in `RealAppKitIntegrationTests`; deciding what to do about the IME case is open. Do not "reconcile" this by asserting the responder chain wins — that was the earlier, wrong version of this rule.
