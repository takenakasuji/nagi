...
**The filename field's `.md` hint hides while `ui.isFilenameComposing`, and `CapturePanel.reportFilenameComposition()` is the only thing that maintains that flag.** The hint's x position is measured from the filename *binding*, which an IME conversion never updates (the same "no `textDidChange` during a conversion" rule below), so mid-conversion the hint sat on top of the reading — and nothing re-rendered, because nothing observable changed. The body solved this with a `setMarkedText` override in `NagiTextView`, but the filename field's editor is SwiftUI's own, so the panel reads `firstResponder`'s `hasMarkedText()` at the only observation points it has: after `sendEvent` and after a consumed key equivalent (both stages report — a ⌘Return consumed at the key-equivalent stage never reaches `sendEvent`), and in `show()`, which reconciles the flag after a conversion ended while the window was hidden and no event carried the news. Safe because `NSTextInputContext` handles events synchronously, so the marked state is settled by the time `sendEvent` returns. The `!(firstResponder is NagiTextView)` exclusion is load-bearing: a conversion in the *body* leaves the filename untouched, and hiding the hint there would blink it on every keystroke of Japanese body text.
