...
使う想定だったが、そのプレースホルダ（「雑に書く。整理はあとで Claude に任せる。」）は
Nagi の主張そのものであり、読ませる必要がある。`#86868B` は白地で 3.62:1 と本文基準を満たさないため、
モックのプレースホルダにも `--label-secondary` を使う。トークンは 2 段階（label / label-secondary）に留める。
